%PLOTROCKETTRAJECTORY  Plot a logged 6DOF run straight from its struct.
%
%   Run this script with the sim output `out` already in the workspace
%   (e.g. out = sim('plant');). Uses out.simout, a struct of nested
%   bus structs, each field a 1x1 timeseries:
%     simout.eom_bus
...
%     numFrames    number of attitude/velocity triads drawn along the
%                  trajectory (default 15)
%     bodyScale    length of the plotted body/velocity vectors, in plot
%                  units (default: auto, ~5% of trajectory span)
...
%       not recomputed here.
%     - velBdy_mps is body-axis velocity (matches its name now): at t=0
%       it equals the 6DOF block's Vm_0 IC directly, with no rotation
%       applied. It is rotated into NED here (via q_na) for the
%       state/trajectory plots, which need NED.

% ---------------------------------------------------------------- options
if ~exist('numFrames','var'),   numFrames = 15;    end
if ~exist('bodyScale','var'),   bodyScale = [];     end
if ~exist('forwardAxis','var'), forwardAxis = [1;0;0]; end

% --------------------------------------------------------- unpack struct
if ~exist('out','var') %#ok<NODEF>
    error('out must be in the workspace, e.g. out = sim(''plant'');');
end
try
    simout = out.simout;
catch
    error('out.simout not found -- out must be the Simulink sim output, e.g. out = sim(''plant'');');
end

requiredBuses = {'eom_bus','aeroParams_bus'};
if ~isstruct(simout) || ~all(isfield(simout, requiredBuses))
    error('out.simout must be a struct with fields eom_bus, aeroParams_bus (each a bus struct).');
end
...
if ~all(isfield(eom, requiredEom))
    error('out.simout.eom_bus must have fields velBdy_mps, posNed_m, q_na, wBdy_rps (each a timeseries).');
end
if ~isfield(aero, 'alpha_rad')
    error('out.simout.aeroParams_bus must have field alpha_rad (a timeseries).');
end
...
if isempty(bodyScale)
    span = max(max(pos_plot) - min(pos_plot));
...
subplot(2,3,1);
plot(t, pos_plot(:,1), t, pos_plot(:,2), t, pos_plot(:,3), 'LineWidth', 1.2);
grid on; xlabel('Time [s]'); ylabel('Position [m]');
legend({'North','East','Up'}, 'Location','best');
title('Position');

subplot(2,3,2);
plot(t, vel_plot(:,1), t, vel_plot(:,2), t, vel_plot(:,3), 'LineWidth', 1.2);
grid on; xlabel('Time [s]'); ylabel('Velocity [m/s]');
legend({'North','East','Up'}, 'Location','best');
title('Velocity');

subplot(2,3,3);
plot(t, tilt_deg, t, roll_deg, 'LineWidth', 1.2);
grid on; xlabel('Time [s]'); ylabel('Angle [deg]');
legend({'Tilt from Vertical','Roll'}, 'Location','best');
title('Tilt & Roll');

subplot(2,3,4);
plot(t, rad2deg(alpha_rad), 'k-', 'LineWidth', 1.5);
grid on; xlabel('Time [s]'); ylabel('Total AoA [deg]');
title('Angle of Attack');

subplot(2,3,5);
plot(t, rad2deg(omega_body(:,1)), t, rad2deg(omega_body(:,2)), t, rad2deg(omega_body(:,3)), 'LineWidth', 1.2);
grid on; xlabel('Time [s]'); ylabel('Angular Rate [deg/s]');
legend({'p','q','r'}, 'Location','best');
title('Omega (Body)');

subplot(2,3,6);
plot(t, q_na(:,1), t, q_na(:,2), t, q_na(:,3), t, q_na(:,4), 'LineWidth', 1.2);
grid on; xlabel('Time [s]'); ylabel('Quaternion');
legend({'q0','q1','q2','q3'}, 'Location','best');
title('Raw Attitude (Quaternion)');
...
plot3(ax, pos_plot(:,1), pos_plot(:,2), pos_plot(:,3), 'b-', 'LineWidth', 1.5);
plot3(ax, pos_plot(1,1), pos_plot(1,2), pos_plot(1,3), 'go', 'MarkerFaceColor','g');
plot3(ax, pos_plot(end,1), pos_plot(end,2), pos_plot(end,3), 'rs', 'MarkerFaceColor','r');

frameIdx = round(linspace(1, N, min(numFrames, N)));
for k = frameIdx
    o = pos_plot(k,:);
    quiver3(ax, o(1), o(2), o(3), nose_plot(k,1), nose_plot(k,2), nose_plot(k,3), ...
        bodyScale, 'r', 'LineWidth', 1.5, 'MaxHeadSize', 0.8);   % body nose axis
    vhat = vel_plot(k,:) / max(norm(vel_plot(k,:)), eps);
    quiver3(ax, o(1), o(2), o(3), vhat(1), vhat(2), vhat(3), ...
        bodyScale, 'Color',[0 0.6 0], 'LineWidth', 1.2, 'MaxHeadSize', 0.8); % velocity dir
end
legend(ax, {'Trajectory','Launch','End','Nose (red)','Velocity (green)'}, 'Location','best');
